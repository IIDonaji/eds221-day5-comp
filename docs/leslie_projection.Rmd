...
# The line below... created a time series to estimate Popu size
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below... Create empty matrix to store population sizes. 
#its getting columns with fish stage and rows with their time point and the value inputted will be Pop size
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below... makes first row initial pop metrics, popu first row with year 0
proj_year[1, ] <- fish_year0

# The for loop below...estimate population size for each year based on popul size in the previous year
for (i in 2:length(time_yr)) { #star at year 1 and beyond
